...
def ExtraConfigDpu(cc: ClustersConfig, cfg: ExtraConfigArgs, futures: dict[str, Future[Optional[host.Result]]]) -> None:
    ...
    logger.info("Building ipu-opi-plugin")
    acc.run("rm -rf /root/ipu-opi-plugins")
    acc.run_or_die(f"git clone {IPU_OPI_PLUGIN_REPO}")
    ret = acc.run_or_die("cat /root/ipu-opi-plugins/ipu-plugin/images/Dockerfile")
    golang_img = extractContainerImage(ret.out)
    acc.run_or_die(f"podman pull docker.io/library/{golang_img}")
    acc.run_or_die("cd /root/ipu-opi-plugins/ipu-plugin && export IMGTOOL=podman && make image")
    vsp_image = f"{registry}/ipu-plugin:acc"
    acc.run_or_die(f"podman tag intel-ipuplugin:latest {vsp_image}")

    render_dpu_vsp_ds(vsp_image, "/tmp/vsp-ds.yaml")
    acc.copy_to("/tmp/vsp-ds.yaml", "/tmp/vsp-ds.yaml")
    client.oc("delete -f /tmp/vsp-ds.yaml")
    client.oc_run_or_die("create -f /tmp/vsp-ds.yaml")

    # build dpu operator images and push to local registry
    logger.info(f"Deploying dpu operator containers on {acc.hostname()}")
    acc.run("rm -rf /root/dpu-operator")
    acc.run_or_die(f"git clone {DPU_OPERATOR_REPO}")

    render_local_images_yaml(operator_image=operator_image, daemon_image=daemon_image, outfilename="/tmp/dpu-local-images.yaml", pull_policy="IfNotPresent")
    acc.copy_to("/tmp/dpu-local-images.yaml", "/root/dpu-operator/config/dev/local-images.yaml")
    acc.run_or_die("pip install yq")
    ensure_go_installed(acc)
    reglocal.local_trust(acc)
    acc.run_or_die(f"podman pull {operator_image}")
    acc.run_or_die(f"podman pull {daemon_image}")
    acc.run(f"cd /root/dpu-operator && export KUBECONFIG={MICROSHIFT_KUBECONFIG} && make undeploy")
    acc.run_or_die(f"cd /root/dpu-operator && export KUBECONFIG={MICROSHIFT_KUBECONFIG} && make local-deploy")
